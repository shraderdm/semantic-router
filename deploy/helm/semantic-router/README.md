...
| dashboard.persistence.enabled | bool | `false` | Persist dashboard-local SQLite state for auth/session/workflow data. This is restart-safe for one dashboard replica, not a shared HA session store. Persistence alone does not keep logins alive across restarts: tokens are signed with dashboard.jwtSecret, which falls back to a random per-start key when unset, so pin both for sessions that survive restarts. |
