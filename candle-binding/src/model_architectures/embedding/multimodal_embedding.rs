...
mod tests {
    ...
    fn test_siglip_image_normalization_constants() {
        ...
        // (x - 0.5) / 0.5. Validate the arithmetic on a known input across
        // the full [0, 1] range, using the IMAGE_MEAN / IMAGE_STD constants
        // and the same broadcast pattern as SigLIPVisionEncoder::forward.
        let device = Device::Cpu;
        // [1, 3, 1, 2] tensor with values 0.0, 0.5, 1.0 to span the range.
        let pixels =
            Tensor::from_slice(&[0.0f32, 1.0, 0.0, 1.0, 0.0, 1.0], (1, 3, 1, 2), &device).unwrap();
        let mean = Tensor::from_slice(&IMAGE_MEAN, (1, 3, 1, 1), &device).unwrap();
        let std = Tensor::from_slice(&IMAGE_STD, (1, 3, 1, 1), &device).unwrap();
        let normalized = pixels.broadcast_sub(&mean).unwrap().broadcast_div(&std).unwrap();
        let out: Vec<f32> = normalized.flatten_all().unwrap().to_vec1().unwrap();
        // Expected per-element: 0.0 -> -1.0, 1.0 -> +1.0, all within tol.
